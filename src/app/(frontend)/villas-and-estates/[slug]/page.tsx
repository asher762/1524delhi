...
export default async function Post({ params: paramsPromise }: Args) {
  ...
  const url = '/villas-and-estates/' + decodedSlug
  const doc = await queryDocBySlug({ slug: decodedSlug })

  if (!doc) return <PayloadRedirects url={url} />

  let relatedDocs = (doc as any).relatedVillasAndEstates
    ...
    <article className="pt-16 pb-16">
      ...
      <div className="flex flex-col items-center gap-4 pt-8">
        <div className="container">
          {doc.content && (
            <RichText className="max-w-3xl mx-auto" data={doc.content} enableGutter={false} />
          )}
